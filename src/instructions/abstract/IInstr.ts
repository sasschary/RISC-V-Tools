...
export abstract class IInstr extends Instruction {
  ...
  public formatCommand(instr: Uint32Array, radix: number = 16): string {
    ...
    if (this.instruction[0] === 'l') {
      // Load instructions get displayed with the whole parenthesis formatting thing
      return `${this.instruction} x${args.rd}, 0x${args.imm.toString(radix)}(x${
        args.rs1
      })`;
    }
    return `${this.instruction} x${args.rd}, x${
      args.rs1
    }, 0x${args.imm.toString(radix)}`;
  }
